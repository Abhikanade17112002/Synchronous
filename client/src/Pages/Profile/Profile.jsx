...
import { getUser, handleUserUpdateProfileAction ,handleRemoveUserProfileImageAction} from "@/store/userSlice/userSlice";
    ...
    <div className="bg-gray-800 min-h-[100vh]  overflow-hidden flex items-center justify-center  flex-col gap-10 ">
      <div className="flex flex-col gap-10 w-[80vw] md:w-max overflow-hidden">
        <div className="">
          ...
          className=" text-3xl overflow-hidden text-white/90 cursor-pointer"></IoArrowBack>
        </div>
        <div className="grid grid-cols-2 justify-center items-center h-full  ">
            ...
            <Avatar  className="h-32 w-32 md:w-48 md:h-48  rounded-full">
              {
                  ...
                  <div className={` uppercase overflow-hidden h-32 w-32 md:w-48 md:h-48 text-5xl border border-black flex justify-center  items-center  rounded-full ${getColor( user?.color || selectedColor)} `}>
                    {
               ...
               hover && (
                <div 
                onClick={ user?.profileimage ? handleUserProfileImageDelete : handleFileInputClicked }
                className="absolute inset-0 flex   items-center justify-center bg-black/50 ring-fuchsia-50 rounded-full">
                  {
                    user?.profileimage  ? <FaTrash className="text-white text-3xl cursor-pointer" /> :<FaPlus  className="text-white text-3xl cursor-pointer" onClick={()=>setFileInput(true)}  />
                  }
                </div>
               )
            }
              <input type="file" ref={fileInputRef} className="hidden" onChange={handleUserProfileImageChange}  name="profileimage" />
            </Avatar>
          ...
          <div className="h-full">
             <form action=""   onSubmit={handleSubmit(handUserDataUpdate)} className=" font-bold h-full flex min-w-32  md:min-w-64 flex-col  text-black items-center justify-evenly">
                ...
                <div className="flex justify-center my-4"> <Button type="submit"  className='mx-3'> Update Profile</Button></div>
             </form>
