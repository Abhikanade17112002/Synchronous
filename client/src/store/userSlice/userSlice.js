  ...
  async (formData) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASEURL}/auth/signin`,
        formData,
        {
          headers: {
...
export const handleUserSignOutAction = createAsyncThunk(
  ...
  async (formData) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASEURL}/auth/signout`,
        {},
        {
          headers: {
            "Content-Type": "application/json",
...
export const handleUserUpdateProfileAction = createAsyncThunk(
  ...
  async (formData) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASEURL}/auth/profile/update`,
        formData,
        {
          headers: {
...
export const handleRemoveUserProfileImageAction = createAsyncThunk(
  "/auth/profile/update/delete",
  async (formData) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASEURL}/auth/profile/update/remove`,
        formData,
        {
          headers: {
            "Content-Type": "application/json",
...
export const getUserInfoAction = createAsyncThunk("/auth/getinfo", async () => {
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_BASEURL}/auth/getinfo`,
      {
        headers: {
          "Content-Type": "application/json",
        },
...
const userSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        handleRemoveUserProfileImageAction.fulfilled,
        (state, action) => {
          state.state = action.payload.status ? "sucess" : "idle";
          state.user = action.payload.status ? action.payload.user : null;
          state.authenticated = action.payload.status;
